...
from schedule_phosphorous_timeseries import load_and_concat_all_sheets_in_centers, clean_data, perform_eda, fit_arima_model, forecast_and_plot

st.set_page_config(page_title="Phosphorous Forecast App", layout="wide")

st.title("Phosphorous Forecast App")
...
st.header(f"Phosphorous Forecast for {selected_location}")
...
plt.plot(location_data['Timestamp'], location_data['Phosphorous01'], marker='o', label=f'Actual {selected_location} Phosphorous')
plt.plot(dates, forecast, linestyle='--', marker='o', color='red', label=f'Forecast {selected_location} Phosphorous')
plt.xlabel('Timestamp')
plt.ylabel('Phosphorous (N)')
...
forecast_df = pd.DataFrame({'Date': dates, 'Forecasted Phosphorous': forecast})
...
st.download_button(
    ...
    data=csv,
    file_name=f"{selected_location}_Phosphorous_forecast.csv",
    mime="text/csv",
)
